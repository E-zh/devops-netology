...
```shell
egor@netology:~/ya-test-04$ terraform workspace list
  default
* prod
  stage
```
2. На всякий случай прилагаю файл тераформа [main.tf](/practice/07.3/main.tf). Вывод команды `terraform plan` для воркспейса `prod`, как видим в данном случае создаются по 2 инстанса:
```shell
egor@netology:~/ya-test-04$ terraform plan
...
Terraform will perform the following actions:
  ...
  # yandex_compute_instance.netology-for["1"] will be created
  ...
  # yandex_compute_instance.netology-for["2"] will be created
  + resource "yandex_compute_instance" "netology-for" {
      + created_at                = (known after apply)
      + folder_id                 = (known after apply)
      + fqdn                      = (known after apply)
